...
def remove_empty(df, labels):
    id_drop = [
        i for i, x in enumerate(df.columns) if df[x].max() == 0
    ]
...
# sort df
def sort_df(df, labels, strand):
    if strand == 'c':
        orders = ['CC', 'CU', 'TA', 'AA', 'AC', 'A', 'C', 'U']
    elif strand == 'g':
        orders = ['TU', 'TA', 'AG', 'GG', 'GU', 'A', 'G', 'U']
    new_labels = []
    for c in orders:
        if c in labels:
            new_labels.append(c)
    df = df.reindex(columns=[x.replace('U', 'T') for x in new_labels])
...
def draw(df, labels, output, no_annot, palette):
    ...
    # draw heatmap
    sns.heatmap(df.T, vmin=0, vmax=0.4, ax=ax, annot=(not no_annot), cmap=palette, annot_kws={"size":font_sizes_in_cell[nbases]})
...
def main():
    ...
    parser.add_argument('--group_size', default=None, choices={4, 16, 64}, help='Force set group size to 4, 16, or 64')
    parser.add_argument('--strand', default=None, choices={'c', 'g'}, help='Telomere strand, {c, g}')
    ...
    # sort df
    if args.strand:
        df, labels = sort_df(df, labels, args.strand)
